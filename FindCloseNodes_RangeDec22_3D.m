function [closeNodes]=FindCloseNodes_RangeDec22_3D(nodeCoordinates,nodeCount)  

%%% 3D find close nodes
voxel=zeros(nodeCount,2);

gap=50e-9;gapSquared=gap^2;
gridsize=1/(4*gap); %setting voxel size
ii=0; jj=0;
...
for s=1:size(occupiedvox,1) %% Loop through occupied voxels
    ...
    closeNodes=[ii(:),jj(:)];
    q=closeNodes(:,1)==closeNodes(:,2);
    closeNodes(q,:)=[];
    qq=closeNodes(:,2)==closeNodes(:,1)+1;
    closeNodes(qq,:)=[];
