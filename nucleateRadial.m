function [elementNodes,ang,rate,nodeCoordinates,nucleationSite,growthNode,ro,phi]= nucleate2CNTs(numberBeamsx,numberBeamsy,diam,length,avgRate,rout,rate_stdev,ang_stdev);

numberBeams=2;
numberBeamsRad=numberBeamsx*numberBeamsy;
radius = diam/2;

for count=1:numberBeamsy
    for o=1:numberBeamsx % uniformly spaced nucleation points along 1/4 perimeter and allows buffer of 3 CNTs at edges
        index=(count-1)*numberBeamsx+o;
        origin(index,1)=radius*cos(-pi/2+2*pi/(numberBeamsRad-1)*(o-1)); %Y-Direction
        origin(index,2)=radius*sin(-pi/2+2*pi/(numberBeamsRad-1)*(o-1)); %Z-Direction
        origin(index,3)=length*(count-1)/numberBeamsy; %Length is in the x-direction
        phi(index)=2*3.14159*rand;
    end
end

   for ii=1:numberBeamsRad ;
     mu_ang = [0];
     Sigma_ang = [(ang_stdev*pi/180)^2;]; 
     R_ang = chol(Sigma_ang);
     rand_ang(ii) = repmat(mu_ang,1) + randn(1)*R_ang;
     phi(ii)=pi/2+ 20*pi/180*rand; %Used to be ang(ii)=
