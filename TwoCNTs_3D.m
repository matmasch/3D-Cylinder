clear
E=1e12; rout=2.5e-9; ri=1.75e-9; A=pi*(rout^2-ri^2); Ic=pi/2*(rout^4-ri^4); EA=E*A; EI=E*Ic; G=5e11; 
...
numberBeamsx=2;% Number of CNTs to be modeled lengthwise
numberBeamsy=1;% Number of CNTs to be modeled breadthwise
numberBeams=2;
ang_stdev=8;% Choose 0,5,10, percent for parametric study
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%[elementNodes,ang,rate,nodeCoordinates,nodeCount,element,nucleationSite,growthNode,ro,rinn,phi]=nucleate2CNTs(numberBeamsx,numberBeamsy,h_span,v_span,avgRate,rout,ri,rate_stdev,ang_stdev);
[elementNodes,ang,rate,nodeCoordinates,nucleationSite,growthNode,ro,phi]= nucleate2CNTs2(rout);
A=pi*(ro.^2-ri.^2)';
Ic=pi/2*(ro.^4-ri.^4)'; 
EA=E.*A; EI=E.*Ic;
L=rate'; plotcount=0;
...
for t=1:steps %% Growing CNT forest for a quantity of "steps" time steps
    ...
    %%closeNodes=FindCNTMeshGrid(nodeCoordinates,nodeCount,numberBeams,t,GDof) 
     ...
     %[gap, sep,closeNodes] =FindCloseNodes3DTruncated(nodeCoordinates,nodeCount,numberBeamsx,numberBeamsy,t,steps);  
...
if ContinuousPlot==1
   ...
   CNTPlot2(fname,avgRate, steps, h_span, numberBeams,ContinuousPlot,nodeCount,nodeCoordinates,t,title,totalCompress, compressiveLoad);
   plotcount=0;
   end
end
...
%save([fullfile(fname,title),'.mat']) % Saves workspace for further manipulation
    if t > 1
    reactionAxial(1,t)=reaction(numberBeams*6*(t-2)+1)%3)
    reactionAxial(2,t)=reaction(numberBeams*6*(t-2)+7)%9);
    end
end


plot(reactionAxial(1,:))
